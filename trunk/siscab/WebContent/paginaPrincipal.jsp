	...
	<tr
		...
		<table border="0" cellpadding="0" cellspacing="0" width="100%">
			...
			<tr>
				...
				<form action="Logoff" onsubmit="fechar()" style="display: inline"><input
					type="image" src="img/bt_sairsistema.gif" name="close"></form>
				</td>
			...
			<tr>
				...
				<fieldset style="width: 750px"><legend>&nbsp;Situação
				de Atendimentos&nbsp;</legend>
				...
				<table>
					<tr>
						...
						<fieldset style="width: 150px"><legend>&nbsp;OBMs
						Ativas&nbsp;</legend>
						<table>
							<tr>
								<td>
								<%
													 ...
													 for(OBM s: obms){
												 		...
												 		{
												 		 	%><a href="#"
									onclick="showLocation2('OBM','><%=s.getLogradouro() %> <%=s.getNumCompl() %> <%=s.getBairro() %> <%=s.getMunicipio().getMunicipio_nome() %> RJ Brasil', 'Nome: <%=s.getNome() %>','Logradouro: <%=s.getLogradouro() %>', 'Nº: <%=s.getNumCompl() %>', 'Bairro: <%=s.getBairro() %>' ,'Município: <%=s.getMunicipio().getMunicipio_nome() %>')"><%=s.getBairro()%></a><br>
								<%
												 		}
												 		else						 			
												 		{
						...
						<fieldset style="width: 200px"><legend>&nbsp;Atendimentos
						Pendentes&nbsp;</legend>
						<table>
							<tr>
								<td>
								...
								<%
													 ...
													 for(Atendimento a: atendimentoAndamento){	
														 //if (a.getStatus_atendimento().equals()
												 		 	%><a href="#"
									onclick="showLocation2('Atendimento em Andamento','><%=a.getLogradouro() %> <%=a.getNumcompl() %> <%=a.getMunicipio_id().getMunicipio_nome() %> RJ Brasil', 'Nº Atendimento: <%=a.getAtendimento_numero() %>','Logradouro: <%=a.getLogradouro() %>', 'Nº: <%=a.getNumcompl() %>', 'Bairro: <%=a.getBairro() %>' ,'Município: <%=a.getMunicipio_id().getMunicipio_nome() %>')"><%=a.getAtendimento_numero()%></a><br>
								<%
													}
													%>
								</td>
				...
				<table>
					<tr>
						...
						<script language="JavaScript" src="js/google.js"></script></td>
						<td valign="top">
						<fieldset style="width: 200px"><legend id="label">Dados
						não selecionados</legend>
						<table>
							<tr>
								<td>
								<div id="nome"></div>
