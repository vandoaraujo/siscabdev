...
public class CronoAtendimentoDao {
	...
	CronoAtendimento crono = (CronoAtendimento) session.load(
		CronoAtendimento.class, id);
	return crono;
    }
    ...
    public List<CronoAtendimento> listaCronologiasReferenteUmAtendimento(
	    int atendimentoId) {

	List<CronoAtendimento> cronoLista = session
		.createQuery(
			"from modelo.CronoAtendimento c where c.atendimento_id=:atendimento")
		.setInteger("atendimento", atendimentoId).list();
	return cronoLista;
    }

    public List<CronoAtendimento> listar() {
	List<CronoAtendimento> crono = session.createQuery(
		"from modelo.CronoAtendimento").list();
	return crono;
    ...
    public CronoAtendimento verificaCronologiaAtendimentoInicio(
	    int atendimentoId) {

	CronoAtendimento cronoInicio = (CronoAtendimento) session
		.createQuery(
			"from modelo.CronoAtendimento c where c.atendimento_id=:atendimento and c.cronoatendimento_tipoevento = 'início'")
		.setInteger("atendimento", atendimentoId).uniqueResult();
	session.close();
	return cronoInicio;
    }
}
