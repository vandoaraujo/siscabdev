...
public class BuscarViatura extends HttpServlet {
	...
	if (status.equals("") && tipoViatura.equals("")) {
	    ...
	    request.setAttribute("parametrosVazios",
		    "Necessário preencher um dos valores para a busca!");
	    ...
	    TipoViatura tv = TipoViaturaDao.getInstance().listarTipoViatura(
		    tipoViatura);
	    viaturas = ViaturaDao.getInstance().listaViaturasPorTipoStatus(
		    tv.getId(), status);
	    ...
	    TipoViatura tv = TipoViaturaDao.getInstance().listarTipoViatura(
		    tipoViatura);
	    viaturas = ViaturaDao.getInstance()
		    .listaViaturasPorTipo(tv.getId());
	} else if ((tipoViatura.equals("")) && !(status.equals(""))) {
