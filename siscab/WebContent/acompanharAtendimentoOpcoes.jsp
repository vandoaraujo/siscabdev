...
<%@ page
	import="java.util.*,modelo.Usuario,modelo.Atendimento, modelo.OBM"%>
	...
	<tr
		...
		<table border="0" cellpadding="0" cellspacing="0" width="100%">
			...
			<tr>
				...
				<%! Atendimento atendimento=null; %> <% atendimento = (Atendimento) request.getSession().getAttribute("atendimentoAtual"); 
				...
				<table>
					<tr>
						<td width="100px">
						<div style="color: red">Municipio:</div>
						</td>
						<td>
						<div style="color: blue"><%= atendimento.getMunicipio_id().getMunicipio_nome() %></div>
						</td>
					</tr>
					<tr>
						<td>
						<div style="color: red">Bairro:</div>
						</td>
						<td>
						<div style="color: blue"><%= atendimento.getBairro() %></div>
						</td>
					</tr>
					<tr>
						<td>
						<div style="color: red">Logradouro:</div>
						</td>
						<td>
						<div style="color: blue"><%= atendimento.getLogradouro() %></div>
						</td>
					</tr>
					<tr>
						<td colspan="2"><!--  <input type="hidden" name="CoordX_Acidente" id="CoordX_Acidente" value="<%= atendimento.getCoordx() %>">
						...
						<input type="submit" value="Editar"
							...
							type="submit" value="Sugestão de Rota"
							onclick="this.form.operacaoARealizar.value=3" /> <input
							type="submit" value="Viaturas Empenhadas"
							onclick="this.form.operacaoARealizar.value=4" /> <input
							type="submit" value="Vitimas"
							onclick="this.form.operacaoARealizar.value=5" /> <input
							type="submit" value="Servicos Executados"
							onclick="this.form.operacaoARealizar.value=6" /> <input
							type="submit" value="Finalizar Atendimento"
							onclick="this.form.operacaoARealizar.value=7" /> <input
							type="hidden" name="operacaoARealizar" value=""> <input
							type="hidden" name="registroAtendimento"
							value="<%= atendimento.getAtendimento_numero() %>" />
